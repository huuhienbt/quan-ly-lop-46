...
function moThongBao() { 
    ...
    if (sortedList.length === 0) listHtml = `<div class="text-center py-10 opacity-60"><i class="fas fa-inbox text-6xl text-slate-300 mb-3"></i><p class="font-bold text-slate-400">Chưa có bài viết nào.</p></div>`; 
        ...
        listHtml = sortedList.map(tb => { 
            ...
            if (tb.time.includes('|||')) { let parts = tb.time.split('|||'); typeStr = parts[0]; displayTime = parts[1]; if(typeStr === 'THÔNG BÁO TỪ GVCN') typeStr = 'Thông báo'; if(typeStr === 'HOẠT ĐỘNG LỚP 4/6') typeStr = 'Hoạt động'; if (typeStr === 'Hoạt động') { iconHtml = '<i class="fas fa-camera-retro"></i>'; colorTheme = 'bg-green-100 text-green-600'; } }
...
function editNotiUI(idToEdit) { 
    ...
    if (isEdit) { if (tb.time.includes('|||')) { let parts = tb.time.split('|||'); defaultType = parts[0]; defaultTimeStr = parts[1]; if(defaultType === 'THÔNG BÁO TỪ GVCN') defaultType = 'Thông báo'; if(defaultType === 'HOẠT ĐỘNG LỚP 4/6') defaultType = 'Hoạt động'; } else { defaultTimeStr = tb.time; } }
