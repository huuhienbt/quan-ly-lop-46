// ==========================================
// NÃO BỘ XỬ LÝ - V68 FINAL (FIX SẮP XẾP THÔNG BÁO CHUẨN NGÀY)
// ==========================================
...
// ==========================================
// 🎯 BẢNG TIN LỚP (BỘ LỌC ĐỌC NGÀY THÁNG THÔNG MINH)
// ==========================================
function moThongBao() {
    closeMenu();
    ...
    // DÒNG CODE VÀNG: Đọc ngày tháng và ép sắp xếp mới nhất lên đầu
    let sortedList = [...Data.notiList].sort((a, b) => {
        const getVal = (item) => {
            // Tìm chuỗi DD/MM/YYYY trong phần thời gian
            const m = String(item.time).match(/(\d{2})\/(\d{2})\/(\d{4})/);
            // Ghép lại thành dạng Năm-Tháng-Ngày (vd: 20260209) để so sánh toán học
            return m ? parseInt(m[3] + m[2] + m[1]) : 0; 
        };
        let dateA = getVal(a), dateB = getVal(b);
        
        // Nếu khác ngày, ngày nào lớn hơn (mới hơn) nằm trên
        if (dateA !== dateB) return dateB - dateA; 
        
        // Nếu cùng ngày, cái nào tạo sau cùng (ID lớn) nằm trên
    ...
    contentArea.innerHTML = `
        <div class="flex items-center mb-6"><button onclick="veTrangChu()" class="bg-white p-2 rounded-xl shadow mr-3"><i class="fas fa-arrow-left text-slate-500"></i></button><h2 class="font-black text-xl text-orange-500 uppercase">BẢNG TIN LỚP</h2></div>
        ${btnTaoMoi}
        <div class="space-y-4 pb-10">${listHtml}</div>
    `;
}
...
window.changeLineSpacing = function(val) {
    ...
    if(sel.rangeCount > 0) {
        ...
        while(node && node.id !== 'frmNotiContent') {
            if(node.nodeName === 'DIV' || node.nodeName === 'P') { node.style.lineHeight = val; break; }
...
function editNotiUI(idToEdit) {
    ...
    contentArea.innerHTML = `
        ...
        <div class="bg-white p-4 sm:p-6 rounded-[2rem] shadow-lg border space-y-4 fade-in w-full overflow-hidden">
            ...
            <div>
                <label class="text-xs font-black text-slate-400 uppercase tracking-wider">Thời gian hiển thị (Giữ đúng định dạng Ngày DD/MM/YYYY)</label>
            ...
            <div class="w-full">
                ...
                <div class="flex flex-wrap gap-2 mb-2 p-2 bg-slate-50 rounded-xl border border-slate-200 items-center">
                    ...
                    <select onchange="changeLineSpacing(this.value)" class="h-8 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded shadow-sm outline-none px-1" title="Giãn dòng">
                        ...
                        <option value="1.2">Nhỏ (1.2)</option>
                        <option value="1.6">Vừa (1.6)</option>
                        <option value="2.0">Rộng (2.0)</option>
                    </select>
...
function chenAnhVaoThongBao() { 
    ...
    if(url) { 
        ...
        document.execCommand('insertHTML', false, htmlAnh); 
    } 
}

function chenFileVaoThongBao() { const url = prompt("Dán đường link (Drive, YouTube...):"); if(url) { const tenLink = prompt("Nhập tên hiển thị:", "Bấm vào đây để xem"); if(tenLink) { document.getElementById("frmNotiContent").focus(); document.execCommand('insertHTML', false, `<a href="${url}" target="_blank">${tenLink}</a>`); } } }
...
async function xoaThongBao(id) {
    if(confirm("Xóa thông báo này vĩnh viễn?")) {
        document.getElementById('loader').style.display = 'flex';
        await fetch(API_URL, { method: 'POST', mode: 'no-cors', body: JSON.stringify({ action: 'xoa_thong_bao', data: { id: id } }) });
        Data.notiList = Data.notiList.filter(x => x.id !== id);
        document.getElementById('loader').style.display = 'none'; moThongBao();
    }
}
