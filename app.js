// ==========================================
// NÃO BỘ XỬ LÝ - V68 BẢN CHUẨN (FIX TRIỆT ĐỂ LỖI VĂNG BÀI THI)
// ==========================================
...
// ==========================================
// 🎯 KHO BÀI TẬP VÀ XỬ LÝ LỖI VĂNG (RACE CONDITION FIX)
// ==========================================
async function loadSubject(sub) { 
    if(!currentUser) return showLogin(); 
    curSub = sub; 
    
    // 1. Tải tức thì bằng Cache nếu có
    if(Data[sub] && Data[sub].length > 0) { 
        renderSubjectData(sub); 
    } else { 
    ...
    // 2. Chạy ngầm cập nhật dữ liệu mới
    try {
        ...
        // 3. KHÓA BẢO VỆ CHỐNG ĐÁ VĂNG: 
        // Chỉ vẽ lại màn hình danh sách bài tập nếu học sinh CHƯA bấm vào làm bài (Không tìm thấy quizBox)
...
async function quanLyNganHang(sub) { closeMenu(); curSub = sub; contentArea.innerHTML = `<div class="text-center mt-10"><i class="fas fa-spinner fa-spin text-3xl text-indigo-600"></i><p class="mt-2 font-bold text-gray-500">Đang tải dữ liệu...</p></div>`; const qs = await (await fetch(API_URL+"?type="+sub+"&t="+Date.now())).json(); Data[sub] = qs; const groups = [...new Set(qs.map(q => q.group))].sort(); let filterOptions = `<option value="all">-- Tất cả các Tuần --</option>` + groups.map(g => `<option value="${g}">${g}</option>`).join(''); contentArea.innerHTML = `<div class="flex items-center justify-between mb-4"><div class="flex items-center"><button onclick="veTrangChu()" class="bg-white p-2 rounded-xl shadow mr-3"><i class="fas fa-arrow-left"></i></button><h2 class="font-black text-xl text-indigo-900 uppercase">KHO ${sub === 'math' ? 'TOÁN' : 'T.VIỆT'}</h2></div><button onclick="renderFormCauHoi(null)" class="bg-indigo-600 text-white px-4 py-2 rounded-xl font-bold btn-3d text-sm"><i class="fas fa-plus mr-1"></i> Tạo câu mới</button></div><select id="qFilter" onchange="filterQuestions()" class="w-full p-3 rounded-xl border-2 border-slate-200 mb-6 font-bold text-slate-700 outline-none focus:border-indigo-500">${filterOptions}</select><div id="listQuestions" class="space-y-4"></div>`; filterQuestions();  }
function filterQuestions() { const val = document.getElementById("qFilter").value; let list = val === 'all' ? Data[curSub] : Data[curSub].filter(q => q.group === val); if (list.length === 0) { document.getElementById("listQuestions").innerHTML = '<p class="text-center text-slate-400 py-10">Trống</p>'; return; } let grouped = {}; list.forEach(q => { if (!grouped[q.group]) grouped[q.group] = []; grouped[q.group].push(q); }); let sortedGroups = Object.keys(grouped).sort((a, b) => { let numA = parseInt(a.replace(/\D/g, '')) || 0; let numB = parseInt(b.replace(/\D/g, '')) || 0; if(numA !== numB) return numA - numB; return a.localeCompare(b); }); let html = ""; sortedGroups.forEach(grp => { let questions = grouped[grp]; html += `<div class="bg-white p-5 rounded-[2rem] shadow-sm border border-slate-100 mb-6 fade-in"><div class="flex items-center justify-between border-b-2 border-indigo-50 pb-3 mb-4"><div class="flex items-center gap-2"><i class="fas fa-layer-group text-indigo-500 text-xl"></i><h3 class="font-black text-lg text-slate-800 uppercase">${grp}</h3></div><span class="bg-indigo-100 text-indigo-700 text-xs font-bold px-3 py-1 rounded-full">${questions.length} câu</span></div><div class="space-y-3">`; questions.forEach((q, index) => { html += `<div class="bg-slate-50 p-4 rounded-xl border border-slate-200 hover:border-indigo-300 transition relative"><div class="flex justify-between items-start"><div class="flex gap-3 w-full pr-16"><span class="w-8 h-8 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold text-sm shrink-0">${index + 1}</span><div class="font-medium text-slate-700 text-base mt-1 overflow-hidden break-words w-full [&_img]:max-w-full [&_img]:h-auto [&_img]:rounded-md [&_img]:mt-2">${parseImg(q.question)}</div></div><div class="flex gap-2 absolute top-4 right-4"><button onclick="renderFormCauHoi('${q.id}')" class="w-8 h-8 bg-blue-100 text-blue-600 rounded-lg flex items-center justify-center hover:bg-blue-600 hover:text-white transition shadow-sm" title="Sửa"><i class="fas fa-edit"></i></button><button onclick="xoaCauHoi('${q.id}')" class="w-8 h-8 bg-red-100 text-red-600 rounded-lg flex items-center justify-center hover:bg-red-600 hover:text-white transition shadow-sm" title="Xóa"><i class="fas fa-trash"></i></button></div></div></div>`; }); html += `</div></div>`; }); document.getElementById("listQuestions").innerHTML = html; }
