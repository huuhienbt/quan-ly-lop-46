// ==========================================
// NÃO BỘ XỬ LÝ - V68 BẢN CHUẨN (THÊM CHỈNH KÍCH THƯỚC ẢNH)
// ==========================================
...
// ==========================================
// 🎯 FORM TẠO BÀI (THÊM NÚT CHỈNH CỠ ẢNH)
// ==========================================
...
// Xử lý khi click vào trong khung soạn thảo
window.handleEditorClick = function(e) {
    // Xóa viền cam của tất cả các ảnh trước
    document.querySelectorAll('#frmNotiContent img').forEach(img => img.style.border = 'none');
    window.currentSelectedImg = null;

    // Nếu click trúng 1 tấm ảnh thì tạo viền báo hiệu
    if (e.target.tagName === 'IMG') {
        e.target.style.border = '3px dashed #f97316'; // Màu cam
        window.currentSelectedImg = e.target;
    }
};

// Hàm đổi kích thước ảnh
...
async function luuThongBaoLenServer() { 
    // Trước khi lưu, phải xóa sạch các viền cam (nếu có) trên ảnh
    document.querySelectorAll('#frmNotiContent img').forEach(img => img.style.border = 'none');
    window.currentSelectedImg = null;
