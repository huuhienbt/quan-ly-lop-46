// ==========================================
// NÃO BỘ XỬ LÝ - V68 FINAL (NÚT ĐĂNG NHẬP TRÊN HEADER)
// ==========================================
...
function setupUI() {
    ...
    // --- XỬ LÝ HEADER (GÓC TRÊN CÙNG BÊN PHẢI) ---
    // Tìm phần tử chứa thông tin user ở góc phải header
    const headerRight = document.querySelector('header > div > div:last-child');
    
    if (currentUser) {
        // NẾU ĐÃ ĐĂNG NHẬP: HIỆN AVATAR VÀ TÊN
        const roleText = currentUser.role === 'admin' ? 'GVCN' : 'Học sinh';
        const userIcon = currentUser.role === 'admin' ? '<i class="fas fa-chalkboard-teacher"></i>' : '<i class="fas fa-user-graduate"></i>';
        
        // Khôi phục lại giao diện User Badge
        headerRight.innerHTML = `
            <div onclick="toggleMenu()" class="flex items-center gap-2 cursor-pointer bg-white/10 px-3 py-1.5 rounded-full hover:bg-white/20 transition">
                <span class="text-right hidden sm:block">
                    <p class="text-[10px] text-blue-200 font-bold uppercase tracking-wider">${roleText}</p>
                    <p class="text-sm font-bold text-white truncate max-w-[100px]" id="headerName">${currentUser.name.split(" ").pop()}</p>
                </span>
                <div class="w-9 h-9 rounded-full bg-white text-blue-600 flex items-center justify-center text-lg shadow-sm border-2 border-blue-200">
                    ${userIcon}
                </div>
            </div>
        `;
        ...
        // Hiện nút đăng xuất trong menu
        const logoutBtn = document.querySelector('[onclick="logout()"]');
        if(logoutBtn) logoutBtn.style.display = '';

    } else {
        // NẾU LÀ KHÁCH: HIỆN NÚT ĐĂNG NHẬP Ở HEADER
        headerRight.innerHTML = `
            <button onclick="showLogin()" class="bg-white text-blue-600 font-black px-4 py-2 rounded-full shadow-lg hover:bg-blue-50 transition text-sm flex items-center gap-2 transform active:scale-95">
        ...
        document.getElementById('menuStudent').innerHTML = `
            <div onclick="showLogin()" class="p-3 hover:bg-slate-50 rounded-lg font-bold flex items-center gap-3 cursor-pointer text-slate-400"><i class="fas fa-rocket w-6"></i> Góc học tập <i class="fas fa-lock ml-auto text-xs"></i></div>
            <div onclick="showLogin()" class="p-3 hover:bg-slate-50 rounded-lg font-bold flex items-center gap-3 cursor-pointer text-slate-400"><i class="fas fa-envelope-open-text w-6"></i> Hộp thư <i class="fas fa-lock ml-auto text-xs"></i></div>
        `;
        document.getElementById('menuStudent').classList.remove('hidden'); 
        document.getElementById('menuTeacher').classList.add('hidden');
        
        // Ẩn nút đăng xuất trong menu
        const logoutBtn = document.querySelector('[onclick="logout()"]');
...
function getNavHtml(active) {
    ...
    // Với Học sinh và Khách: Chỉ hiện tên nếu là Học sinh
    let headerGreeting = "";
    if (currentUser && currentUser.role === 'student') {
        headerGreeting = `<div class="mb-5 fade-in"><h2 class="text-2xl font-black text-slate-800">Chào, ${currentUser.name}!</h2></div>`;
    } else if (!currentUser) {
        // Khách thì hiện tiêu đề Lớp 4/6
        headerGreeting = `<div class="mb-5 fade-in"><h2 class="text-2xl font-black text-blue-600">TRANG CHỦ LỚP 4/6</h2></div>`;
    }
    ...
    <div class="flex items-center gap-6 sm:gap-10 mb-6 border-b-2 border-slate-100 pb-2 overflow-x-auto whitespace-nowrap scrollbar-hide">
        <button onclick="moThongBao()" class="font-black text-base sm:text-xl pb-2 transition ${active==='bangtin' ? 'text-orange-500 border-b-4 border-orange-500' : 'text-slate-400 hover:text-orange-500'}">BẢNG TIN LỚP</button>
        <button onclick="moGocHocTap()" class="font-black text-base sm:text-xl pb-2 transition ${active==='hoctap' ? 'text-indigo-600 border-b-4 border-indigo-500' : 'text-slate-400 hover:text-indigo-500'}">GÓC HỌC TẬP</button>
...
// ==========================================
// 🎯 BẢNG TIN (KHÔNG CÒN NÚT ĐĂNG NHẬP Ở BANNER)
// ==========================================
function moThongBao() { 
    closeMenu(); 
    ...
    // Banner chào phụ huynh (Đã bỏ nút đăng nhập)
    let guestNotice = !currentUser ? `<div class="bg-gradient-to-br from-indigo-500 to-blue-600 text-white p-5 rounded-3xl shadow-lg mb-6 fade-in"><div class="flex items-center gap-3 mb-2"><i class="fas fa-school text-3xl text-white/80"></i><h3 class="font-black text-xl tracking-wide">Xin chào Phụ huynh Lớp 4/6</h3></div><p class="text-sm text-blue-100 opacity-90 leading-relaxed">Đây là kênh thông tin chính thức. Quý phụ huynh vui lòng <b class="text-white">Đăng nhập</b> ở góc trên bên phải để sử dụng đầy đủ các tính năng.</p></div>` : '';
    ...
    contentArea.innerHTML = `${guestNotice}${getNavHtml('bangtin')}${btnTaoMoi}<div class="space-y-4 pb-10">${listHtml}</div>`; 
}
