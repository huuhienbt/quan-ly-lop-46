...
// Cập nhật: Khắc phục lỗi khựng Vòng quay (Tải ngầm chống lách luật)
// ==========================================
...
function logout() { localStorage.removeItem('role'); localStorage.removeItem('uid'); location.reload(); }
...
// ----------------------------------------------------
// TÍNH NĂNG ĐIỀU CHỈNH THANH MENU (ĐÃ THÊM ICON)
// ----------------------------------------------------
...
// ==========================================
// 🎡 TÍNH NĂNG VÒNG QUAY MAY MẮN (Background Fetch Chống Khựng & Lách Luật)
// ==========================================
const PRIZES = [
...
async function moVongQuay() {
    ...
    // Mặc định cho hiển thị trạng thái nút Tải/Kiểm tra để chống khựng
    let btnText = "ĐANG TẢI...";
    ...
    // HIỂN THỊ GIAO DIỆN NGAY LẬP TỨC (MƯỢT MÀ 100%)
    contentArea.innerHTML = `
        ${getNavHtml('vongquay')}
    ...
    // 2. CHẠY NGẦM VIỆC TẢI DỮ LIỆU ĐỂ KIỂM TRA CHỐNG ẨN DANH (Không làm chặn giao diện)
    ...
    // BẬT SÁNG NÚT BẤM SAU KHI KIỂM TRA XONG
...
async function thucHienQuay() {
    ...
    setTimeout(async () => {
        ...
        try {
            await fetch(API_URL, { 
                method: 'POST', body: JSON.stringify({ action: 'nop_bai', data: { id_hs: currentUser.id, subject: "LuckySpin", group: uniqueGroup, score_earned: prize.netScore, details: "Quay trúng: " + prize.text } }) 
            });
...
async function moQuanLyThu() {
    closeMenu(); contentArea.innerHTML = `<h2 class="text-xl font-black text-pink-600 mb-4 text-center"><i class="fas fa-spinner fa-spin"></i> Đang mở hộp thư...</h2>`;
    try {
        const letters = await (await fetch(API_URL + "?type=mailbox&t=" + Date.now())).json(); let html = `<div class="flex items-center mb-6"><button onclick="veTrangChu()" class="bg-white p-2 rounded-xl shadow mr-3"><i class="fas fa-arrow-left text-slate-500"></i></button><h2 class="font-black text-xl text-pink-600 uppercase">THƯ TỪ HỌC SINH</h2></div>`;
        if (letters.length === 0) { html += `<p class="text-center text-slate-400 font-medium py-10"><i class="fas fa-comment-dots text-4xl mb-3 text-pink-200 block"></i>Hộp thư đang trống, chưa có em nào gửi.</p>`; } 
        else {
            ...
            letters.forEach(l => {
                ...
                let isAnon = (l.isAnonymous === "TRUE" || l.isAnonymous === true || l.isAnonymous === "true"); let senderDisplay = isAnon ? `<span class="text-purple-600"><i class="fas fa-user-secret"></i> Ẩn danh (Thực tế: ${l.name})</span>` : `<span class="text-blue-600"><i class="fas fa-user"></i> ${l.name}</span>`; let anonBadge = isAnon ? `<span class="bg-purple-100 text-purple-700 text-[10px] font-black px-2 py-1 rounded ml-2">THƯ ẨN DANH</span>` : '';
    ...
    } catch (e) { contentArea.innerHTML = `<p class="text-center text-red-500 mt-10">Lỗi tải dữ liệu hộp thư.</p>`; }
}
...
async function dongBoDuLieu() { 
    ...
    try { await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'clear_cache', data: {} }) }); alert("Đồng bộ thành công! Giao diện sẽ tự động tải lại dữ liệu mới."); location.reload(); } catch(e) { document.getElementById('loader').style.display = 'none'; alert("Lỗi mạng khi đồng bộ!"); } 
}
