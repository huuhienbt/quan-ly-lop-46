// ==========================================
// NÃO BỘ XỬ LÝ - V68 (LƯU THÔNG BÁO LÊN CLOUD)
// ==========================================
...
window.onload = async () => {
    try {
        // Tải danh sách HS và Thông báo cùng lúc cho nhanh
    ...
    contentArea.innerHTML = `
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 fade-in">
            <button onclick="moThongBao()" class="col-span-1 md:col-span-2 relative h-32 bg-gradient-to-r from-orange-400 to-amber-500 rounded-3xl shadow-lg flex items-center p-6 btn-3d border-2 border-white overflow-hidden group">
                ...
                <span class="absolute top-4 right-4 w-4 h-4 bg-red-500 rounded-full animate-ping border-2 border-white"></span>
            </button>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center gap-3 mb-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs font-bold text-slate-400 mt-0.5"><i class="fas fa-clock mr-1"></i> ${Data.noti.time}</p>
                    </div>
                </div>
            </div>
            <div class="bg-orange-50 p-5 rounded-2xl text-slate-700 font-medium leading-relaxed whitespace-pre-wrap min-h-[120px] border border-orange-100 text-lg">${Data.noti.content}</div>
            ${btnSua}
        </div>
    `;
}

async function editNoti() {
    const n = prompt("Nhập nội dung thông báo mới:", Data.noti.content === "Hiện tại chưa có thông báo mới từ GVCN." ? "" : Data.noti.content);
    if(n !== null && n.trim() !== "") { 
        const d = new Date();
        ...
        const timeStr = `Ngày ${dateStr} - Tuần ${weekNum}`;

        document.getElementById('loader').style.display = 'flex';
        
        // Gửi thông báo lên Google Sheet
        await fetch(API_URL, { 
            method: 'POST', mode: 'no-cors', 
            body: JSON.stringify({ action: 'dang_thong_bao', data: { time: timeStr, content: n } }) 
        });

        // Cập nhật giao diện
        Data.noti = { time: timeStr, content: n };
        document.getElementById('loader').style.display = 'none';
        alert("Đăng thông báo thành công! Học sinh sẽ nhìn thấy ngay.");
        moThongBao(); 
    }
}

// CÁC HÀM CŨ GIỮ NGUYÊN (Góc học tập, Quản lý HS, Zalo, Đơn từ...)
    ...
    if(grps.length === 0) html += `<p class="text-center text-gray-400 mt-10">Hiện chưa có bài tập nào.</p>`;
        ...
        grps.forEach(g => {
            ...
            <div onclick="startQuiz('${g}', ${time})" class="bg-white p-4 rounded-2xl border-2 flex justify-between items-center cursor-pointer hover:-translate-y-1 transition btn-3d ${isDone ? 'border-green-100 bg-green-50/30' : 'border-indigo-50'}">
                <div class="flex items-center gap-4"><div class="w-12 h-12 rounded-xl flex items-center justify-center text-2xl ${isDone ? 'bg-green-100 text-green-600' : 'bg-indigo-100 text-indigo-600'}"><i class="fas ${isDone ? 'fa-check' : 'fa-star'}"></i></div><div><h3 class="font-black text-lg text-slate-700">${g}</h3><p class="text-xs font-bold text-slate-400 mt-1"><i class="fas fa-clock mr-1"></i>${time} phút • ${count} câu</p></div></div>
...
function renderQuizFrame() {
    contentArea.innerHTML = `
        <div class="sticky top-20 bg-[#f0f7ff] z-30 py-3 flex justify-between items-center mb-4"><div class="flex items-center gap-3"><button onclick="loadSubject(curSub)" class="bg-white w-10 h-10 rounded-full shadow-sm text-slate-500 font-bold border"><i class="fas fa-times"></i></button><span class="font-black text-indigo-900 truncate max-w-[150px]">${curGrp}</span></div><div class="bg-white px-4 py-2 rounded-full font-black text-indigo-600 shadow-sm border border-indigo-100 flex items-center gap-2"><i class="fas fa-stopwatch text-orange-500 animate-pulse"></i><span id="quizTimer">00:00</span></div></div>
...
function checkAns(el, selected, correct, index) {
    ...
    else el.classList.add('!bg-red-100', '!border-red-500', '!text-red-800');
...
async function finishQuiz() {
    ...
    if(currentUser.role === 'student') {
        await fetch(API_URL, { method: 'POST', mode: 'no-cors', body: JSON.stringify({ action: 'nop_bai', data: { id_hs: currentUser.id, subject: curSub, group: curGrp, score_earned: score } }) });
...
async function quanLyNganHang(sub) {
    ...
    contentArea.innerHTML = `<div class="flex items-center justify-between mb-4"><div class="flex items-center"><button onclick="veTrangChu()" class="bg-white p-2 rounded-xl shadow mr-3"><i class="fas fa-arrow-left"></i></button><h2 class="font-black text-xl text-indigo-900 uppercase">KHO ${sub === 'math' ? 'TOÁN' : 'T.VIỆT'}</h2></div><button onclick="renderFormCauHoi(null)" class="bg-indigo-600 text-white px-4 py-2 rounded-xl font-bold btn-3d text-sm"><i class="fas fa-plus mr-1"></i> Tạo mới</button></div><select id="qFilter" onchange="filterQuestions()" class="w-full p-3 rounded-xl border-2 border-slate-200 mb-6 font-bold text-slate-700 outline-none focus:border-indigo-500">${filterOptions}</select><div id="listQuestions" class="space-y-4"></div>`;
    filterQuestions(); 
}
...
async function xoaCauHoi(id) {
    if(confirm("Thầy có chắc chắn muốn xóa câu hỏi này không?")) {
        ...
        alert("Đã xóa!"); document.getElementById('loader').style.display = 'none'; quanLyNganHang(curSub);
    }
}
...
async function sendLeave() {
    ...
    document.getElementById('loader').style.display='flex';
    try { await fetch(API_URL, { method:'POST', mode:'no-cors', body:JSON.stringify({ action:'gui_xin_phep', data:{ id:currentUser.id, name:currentUser.name, dateOff:d, type:'Nghỉ cả ngày', reason:r } }) }); alert("Gửi đơn thành công!"); veTrangChu(); } 
