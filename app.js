// ==========================================
// NÃO BỘ XỬ LÝ - APP.JS (BẢN CHUẨN 100% - KHÔNG CẮT XÉN)
// Tích hợp: Vòng quay 6 ô (Vé làm lại, Câu đố), Đọc hiểu Tiếng Việt 2 khung, Soạn Hình học
// ==========================================
...
window.onload = async () => {
    ...
    try {
        ...
        document.getElementById('connStatus').className = "mt-4 text-xs font-bold text-red-500"; loginGuest(); 
    }
};
...
// BỘ TẠO THANH CÔNG CỤ (TOOLBAR) SOẠN THẢO CHUẨN QUỐC TẾ (HỖ TRỢ CHÈN ẢNH)
function getRichTextToolbar(targetId) {
    return `
        <div class="flex flex-wrap gap-2 mb-2 p-2 bg-slate-50 rounded-xl border border-slate-200 items-center shadow-sm">
            ...
            <div class="w-px h-6 bg-slate-300 mx-1"></div>
            <button onclick="chenAnhVaoEditor('${targetId}')" class="px-3 h-8 bg-indigo-50 rounded shadow-sm hover:bg-indigo-100 text-indigo-700 font-bold text-xs flex items-center gap-1 border border-indigo-200"><i class="fas fa-image"></i> Chèn Ảnh</button>
        </div>
    `;
}

// HÀM CHÈN ẢNH VÀO BẤT KỲ EDITOR NÀO
...
// ==========================================
// 🎡 TÍNH NĂNG VÒNG QUAY MAY MẮN (CẬP NHẬT 6 Ô MỚI)
// ==========================================
const PRIZES = [
    ...
    { id: "redo", text: "Vé Làm Lại", color: "#fb923c", netScore: 0, extraSpin: 0, msg: "Con nhận được 1 VÉ LÀM LẠI BÀI. Hãy dùng nó để làm lại một bài tập bị điểm thấp nhé!", icon: "🎫" },
    ...
    let slicesHtml = PRIZES.map((p, i) => {
        return `
            <div class="absolute inset-0 flex justify-center" style="transform: rotate(${i * 60}deg);">
                <div class="pt-5 font-black text-white text-xs sm:text-sm drop-shadow-md w-16 text-center leading-tight z-20" style="transform: rotate(0deg);">
                    ${p.text}
                </div>
            </div>
        `;
        ...
        <div class="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 text-center fade-in">
            ...
            <div class="relative w-64 h-64 sm:w-80 sm:h-80 mx-auto mb-8">
                ...
                <div id="wheel" class="w-full h-full rounded-full border-8 border-yellow-400 shadow-2xl relative overflow-hidden" 
                     style="background: conic-gradient(from -30deg, ${gradColors}); transition: transform 4s cubic-bezier(0.17, 0.67, 0.12, 0.99);">
                    ${slicesHtml}
...
async function thucHienQuay() {
    ...
    // Thuật toán Tỉ lệ: 10% (+10đ), 20% (+Lượt), 20% (Câu đố), 10% (-10đ), 10% (Vé), 30% (May mắn)
    let rand = Math.random() * 100;
    let idx = 0;
    if (rand < 10) idx = 0;                 
    else if (rand < 30) idx = 1;            
    else if (rand < 50) idx = 2;            
    else if (rand < 60) idx = 3;            
    else if (rand < 70) idx = 4;            
    else idx = 5;                           
    ...
    setTimeout(async () => {
        ...
        // Xử lý ô Giải Đố
        if (prize.id === 'riddle') {
            showRiddleModal(prize, todayStr, uniqueGroup);
            return; // Dừng tại đây, việc lưu điểm sẽ làm sau khi giải đố xong
        }
...
function restoreSpinButton(spinLog) {
    ...
    if (canSpinNow) {
        ...
        btnUpdate.innerText = spinLog.usedFree ? `QUAY (+${spinLog.extra} LƯỢT)` : "QUAY MIỄN PHÍ";
    } else {
...
// KHUNG GIAO DIỆN CÂU ĐỐ NHẬN THƯỞNG
function showRiddleModal(prizeTemplate, todayStr, uniqueGroup) {
    let a = Math.floor(Math.random() * 10) + 1;
    let b = Math.floor(Math.random() * 10) + 1;
    let c = Math.floor(Math.random() * 10) + 1;
    let correctAns = a + b * c; // Luật nhân chia trước cộng trừ sau
...
window.checkRiddle = async function(correctAns, todayStr, uniqueGroup) {
    ...
    if (parseInt(ansStr) === correctAns) {
        prize.netScore = 20;
        prize.msg = "Giỏi quá! Con đã tính đúng và được thưởng 20 điểm.";
        prize.icon = "🎉"; prize.color = "#34d399";
    } else {
        prize.msg = `Tiếc quá! Đáp án đúng là ${correctAns}. Con không nhận được điểm, hãy cẩn thận hơn ở lần sau nhé!`;
        prize.icon = "😅"; prize.color = "#f87171";
    }
...
function editNotiUI(idToEdit) { 
    ...
    contentArea.innerHTML = `<div class="flex items-center mb-6"><button onclick="moThongBao()" class="bg-white p-2 rounded-xl shadow mr-3 text-slate-500"><i class="fas fa-times"></i></button><h2 class="font-black text-xl text-orange-600 uppercase">${isEdit ? 'SỬA BÀI VIẾT' : 'TẠO BÀI VIẾT MỚI'}</h2></div><div class="bg-white p-4 sm:p-6 rounded-[2rem] shadow-lg border space-y-4 fade-in w-full overflow-hidden"><input type="hidden" id="frmNotiId" value="${idToEdit || ''}"><div class="grid grid-cols-1 sm:grid-cols-2 gap-4"><div><label class="text-xs font-black text-slate-400 uppercase tracking-wider block mb-1">Loại bài viết</label><select id="frmNotiType" class="edit-input w-full bg-slate-50 border-2 border-slate-200 p-3 rounded-xl font-bold text-slate-700 outline-none focus:border-orange-400 transition"><option value="Thông báo" ${defaultType === 'Thông báo' ? 'selected' : ''}>📣 Thông báo</option><option value="Hoạt động" ${defaultType === 'Hoạt động' ? 'selected' : ''}>📸 Hoạt động</option></select></div><div><label class="text-xs font-black text-slate-400 uppercase tracking-wider block mb-1">Thời gian hiển thị</label><input type="text" id="frmNotiTime" class="edit-input w-full bg-slate-50 border-2 border-slate-200 p-3 rounded-xl font-bold text-slate-700 outline-none focus:border-orange-400" value="${defaultTimeStr}"></div></div><div class="w-full"><label class="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Nội dung</label><div class="flex flex-wrap gap-2 mb-2 p-2 bg-slate-50 rounded-xl border border-slate-200 items-center"><button onclick="document.execCommand('bold', false, null)" class="w-8 h-8 bg-white rounded shadow-sm hover:bg-slate-200 font-black">B</button><button onclick="document.execCommand('italic', false, null)" class="w-8 h-8 bg-white rounded shadow-sm hover:bg-slate-200 italic font-serif">I</button><div class="relative flex items-center bg-white rounded shadow-sm px-1 hover:bg-slate-200 h-8" title="Màu chữ"><input type="color" onchange="document.execCommand('foreColor', false, this.value)" class="w-5 h-5 border-0 bg-transparent cursor-pointer"></div><div class="w-px h-6 bg-slate-300 mx-1"></div><button onclick="document.execCommand('justifyLeft', false, null)" class="w-8 h-8 bg-white rounded shadow-sm hover:bg-slate-200 text-slate-600" title="Căn trái"><i class="fas fa-align-left"></i></button><button onclick="document.execCommand('justifyCenter', false, null)" class="w-8 h-8 bg-white rounded shadow-sm hover:bg-slate-200 text-slate-600" title="Căn giữa"><i class="fas fa-align-center"></i></button><button onclick="document.execCommand('justifyRight', false, null)" class="w-8 h-8 bg-white rounded shadow-sm hover:bg-slate-200 text-slate-600" title="Căn phải"><i class="fas fa-align-right"></i></button><button onclick="document.execCommand('justifyFull', false, null)" class="w-8 h-8 bg-white rounded shadow-sm hover:bg-slate-200 text-slate-600" title="Căn đều 2 bên"><i class="fas fa-align-justify"></i></button><div class="w-px h-6 bg-slate-300 mx-1"></div><select onchange="document.execCommand('fontSize', false, this.value)" class="h-8 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded shadow-sm outline-none px-1"><option value="">Cỡ chữ</option><option value="1">Rất nhỏ</option><option value="2">Nhỏ</option><option value="3">Vừa</option><option value="4">Lớn</option><option value="5">Rất Lớn</option><option value="6">Khổng lồ</option></select><select onchange="changeLineSpacing(this.value)" class="h-8 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded shadow-sm outline-none px-1"><option value="">Giãn dòng</option><option value="1.2">Nhỏ (1.2)</option><option value="1.6">Vừa (1.6)</option><option value="2.0">Rộng (2.0)</option></select><div class="w-px h-6 bg-slate-300 mx-1"></div><button onclick="chenAnhVaoEditor('frmNotiContent')" class="px-2 h-8 bg-white rounded shadow-sm hover:bg-slate-200 text-indigo-600 font-bold text-xs flex items-center gap-1"><i class="fas fa-upload"></i> Thêm Ảnh</button><button onclick="chenFileVaoThongBao()" class="px-2 h-8 bg-white rounded shadow-sm hover:bg-slate-200 text-blue-600 font-bold text-xs flex items-center gap-1"><i class="fas fa-link"></i> Link</button><select onchange="resizeImg(this.value); this.value='';" class="h-8 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded shadow-sm outline-none px-1"><option value="">Cỡ ảnh</option><option value="30%">Nhỏ (30%)</option><option value="60%">Vừa (60%)</option><option value="100%">Lớn (100%)</option></select></div><div id="frmNotiContent" onclick="handleEditorClick(event)" contenteditable="true" class="w-full min-h-[200px] bg-white border-2 border-slate-200 p-4 rounded-xl outline-none focus:border-orange-400 transition text-base overflow-hidden break-words [&_img]:max-w-full [&_img]:h-auto [&_img]:rounded-lg [&_img]:inline-block [&_img]:cursor-pointer [&_a]:text-blue-600 [&_a]:underline">${currentContent}</div></div><button onclick="luuThongBaoLenServer()" class="w-full bg-orange-600 text-white py-4 rounded-2xl font-black btn-3d shadow-lg mt-2 text-lg hover:bg-orange-700 transition">${isEdit ? 'LƯU THAY ĐỔI' : 'ĐĂNG LÊN HỆ THỐNG'}</button></div>`; 
}
...
async function luuThongBaoLenServer() { document.querySelectorAll('#frmNotiContent img').forEach(img => img.style.border = 'none'); window.currentSelectedImg = null; const id = document.getElementById("frmNotiId").value; const typeStr = document.getElementById("frmNotiType").value; const timeVal = document.getElementById("frmNotiTime").value; const finalTimeStr = typeStr + "|||" + timeVal; const contentHTML = document.getElementById("frmNotiContent").innerHTML; if(!timeVal || !contentHTML.trim()) return alert("Vui lòng nhập nội dung!"); document.getElementById('loader').style.display = 'flex'; try { const act = id ? 'sua_thong_bao' : 'dang_thong_bao'; await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: act, data: { id: id, time: finalTimeStr, content: contentHTML } }) }); if(id) { const idx = Data.notiList.findIndex(x => x.id === id); if(idx > -1) { Data.notiList[idx].time = finalTimeStr; Data.notiList[idx].content = contentHTML; } } else { Data.notiList.unshift({ id: "TB"+Date.now(), time: finalTimeStr, content: contentHTML }); } document.getElementById('loader').style.display = 'none'; alert("Đăng bài thành công!"); moThongBao(); } catch(e) { document.getElementById('loader').style.display = 'none'; alert("Lỗi mạng!"); } }
...
async function moGocHocTap() { 
    ...
    // LỌC THỜI GIAN ĐỂ XẾP HẠNG (Chỉ lấy log nộp bài hoặc log có cộng/trừ điểm)
    ...
    // SẮP XẾP: Điểm cao lên trên -> Bằng điểm thì ai có thời gian NHỎ HƠN (Sớm hơn) lên trên
    ...
    if (top15.length > 0) { 
        let listHtml = top15.map((s) => { 
            let actualDisplayRank = uniqueScores.indexOf(s.score) + 1; // Hạng đồng điểm
...
// BỘ TẠO THANH CÔNG CỤ (TOOLBAR) SOẠN THẢO CHUẨN QUỐC TẾ (CHÈN ĐƯỢC ẢNH)
function getRichTextToolbar(targetId) {
    return `
        <div class="flex flex-wrap gap-2 mb-2 p-2 bg-slate-50 rounded-xl border border-slate-200 items-center shadow-sm">
            <button onclick="document.execCommand('bold', false, null)" class="w-8 h-8 bg-white rounded shadow-sm hover:bg-slate-200 font-black text-slate-700">B</button>
...
function renderFormCauHoi(id) { 
    ...
    // GIAO DIỆN TÁCH 2 KHUNG CHO TIẾNG VIỆT
    ...
    if (isTV) {
        formLayout = `
            <div class="mb-4 p-4 bg-yellow-50 border-2 border-yellow-200 rounded-2xl">
                ...
                <div id="frmBaiDoc" contenteditable="true" class="w-full min-h-[120px] bg-white border border-yellow-300 p-4 rounded-xl outline-none focus:border-orange-400 transition text-base overflow-hidden break-words [&_img]:max-w-full [&_img]:rounded-md">${baiDocHtml}</div>
            </div>
            <div class="w-full mb-4">
                ...
                <div id="frmQ" contenteditable="true" class="w-full min-h-[120px] bg-white border-2 border-green-200 p-4 rounded-xl outline-none focus:border-green-500 transition text-base overflow-hidden break-words [&_img]:max-w-full [&_img]:rounded-md">${cauHoiHtml}</div>
            </div>
        `;
    } else {
        formLayout = `
            <div class="w-full mb-4">
                ...
                ${getRichTextToolbar('frmQ')}
                <div id="frmQ" contenteditable="true" class="w-full min-h-[150px] bg-white border-2 border-indigo-200 p-4 rounded-xl outline-none focus:border-indigo-500 transition text-base overflow-hidden break-words [&_img]:max-w-full [&_img]:rounded-md">${cauHoiHtml}</div>
            </div>
        `;
    }
    ...
    contentArea.innerHTML = `
        ...
        <div class="bg-white p-5 rounded-3xl shadow border space-y-4 fade-in">
            <div class="bg-yellow-50 p-3 rounded-xl border border-yellow-200 text-xs font-bold text-yellow-700 mb-2">
                <i class="fas fa-info-circle"></i> MẸO: Chọn nút <i class="fas fa-image text-indigo-500"></i> để chèn hình ảnh vào nội dung câu hỏi một cách dễ dàng!
            </div>
            ${dl}
...
async function luuCauHoi(id) { 
    ...
    window.isQuizDataLoaded = false; 
    alert("Lưu thành công!"); document.getElementById('loader').style.display = 'none'; quanLyNganHang(curSub, true); 
}
...
async function xoaCauHoi(id) { 
    if(confirm("Thầy có chắc chắn muốn xóa câu hỏi này không?")) { 
        ...
        window.isQuizDataLoaded = false; alert("Đã xóa!"); document.getElementById('loader').style.display = 'none'; quanLyNganHang(curSub, true); 
    } 
}

// --- LÀM BÀI TẬP HỌC SINH (CHIA ĐÔI MÀN HÌNH & XÀI VÉ LÀM LẠI) ---
        ...
        grps.forEach(g => { 
            ...
            if (isDone && currentUser.role === 'student') { 
                ...
                if(tokens > 0) {
                    ...
                    clickAction = `alert('Con đã hoàn thành bài tập này. Quay Vòng quay Nhân Phẩm để tìm VÉ LÀM LẠI nhé!')`; 
                }
            } 
...
window.promptRedo = function(group, time) {
    ...
    if(tokens > 0) {
        if(confirm(`Con đang có ${tokens} VÉ LÀM LẠI.\nCon có chắc chắn muốn dùng 1 vé để mở khóa và làm lại [${group}] không?`)) {
            localStorage.setItem('redo_tokens_'+currentUser.id, tokens - 1);
            // Xóa log cũ ảo trên bộ nhớ máy (để UI ko bị lỗi), Server sẽ nhận log mới
...
function startQuiz(group, timeMins) { 
    ...
    // BÓC TÁCH BÀI ĐỌC (TIẾNG VIỆT) VÀ CÂU HỎI
    rawQuiz.forEach(q => {
...
function checkAns(el, selected, correct, index) { 
    ...
    if (selected === correct.toLowerCase()) { 
        ...
        let qText = parseImg(q.question); let wrongAnsText = parseImg(q[selected]); let correctAnsText = parseImg(q[correct]); 
        let wrongText = `<div class="bg-white p-4 rounded-xl border border-red-200 mb-3 shadow-sm"><p class="font-bold text-slate-800 border-b border-slate-100 pb-2 mb-2"><span class="text-red-500">Câu ${index+1}:</span> ${qText}</p><div class="space-y-2 mt-3"><p class="text-red-600 text-sm bg-red-50 p-2 rounded-lg border border-red-100"><i class="fas fa-times-circle mr-1"></i> <b>Bé chọn (${selected.toUpperCase()}):</b> <span class="font-medium">${wrongAnsText}</span></p><p class="text-green-600 text-sm bg-green-50 p-2 rounded-lg border border-green-100"><i class="fas fa-check-circle mr-1"></i> <b>Đáp án đúng (${correct.toUpperCase()}):</b> <span class="font-medium">${correctAnsText}</span></p></div></div>`; 
