// ==========================================
// NÃO BỘ XỬ LÝ - V68 BẢN FINAL (SOẠN THẢO THÔNG BÁO NÂNG CAO)
// ==========================================
...
let Data = { hs: [], math: [], tv: [], log: [], stats: null, leaves: [], noti: {time: "", content: "Đang tải thông báo..."} };
...
window.onload = async () => {
    try {
        ...
        Data.hs = await resHs.json(); Data.noti = await resNoti.json();
...
function renderDashboardStudent() {
    const hasNoti = Data.noti.content && Data.noti.content !== "Hiện tại chưa có thông báo mới từ GVCN.";
    contentArea.innerHTML = `
...
// ==========================================
// 🎯 TRÌNH SOẠN THẢO THÔNG BÁO NÂNG CAO (WORD THU NHỎ)
// ==========================================
function moThongBao() {
    closeMenu();
    let btnSua = currentUser.role === 'admin' ? `<button onclick="editNotiUI()" class="w-full mt-6 bg-orange-600 text-white py-4 rounded-2xl font-black shadow-lg btn-3d hover:bg-orange-700 transition"><i class="fas fa-edit mr-2"></i> SOẠN THÔNG BÁO MỚI</button>` : '';

    // Dùng CSS Tailwind để khống chế ảnh và link trong nội dung thông báo không bị tràn màn hình
    contentArea.innerHTML = `
        ...
        <div class="bg-white p-6 rounded-[2rem] shadow-lg border-t-4 border-orange-400 fade-in">
            <div class="flex flex-col sm:flex-row sm:items-center gap-3 mb-4">
                <div class="flex items-center gap-3">
                    <div class="w-12 h-12 bg-orange-100 text-orange-500 rounded-full flex items-center justify-center text-2xl shrink-0"><i class="fas fa-bell"></i></div>
                    <div>
                        <h3 class="font-bold text-slate-800 text-lg">Tin nhắn từ GVCN</h3>
                        <p class="text-xs font-bold text-slate-400 mt-0.5"><i class="fas fa-clock mr-1"></i> ${Data.noti.time || "Đang chờ cập nhật..."}</p>
                    </div>
                </div>
            </div>
            <div class="bg-orange-50/50 p-5 rounded-2xl text-slate-800 text-lg border border-orange-100 min-h-[150px] [&_img]:max-w-full [&_img]:rounded-xl [&_img]:shadow-sm [&_img]:my-3 [&_a]:text-blue-600 [&_a]:underline [&_a]:font-bold leading-relaxed">
                ${Data.noti.content}
            </div>
            ${btnSua}
        </div>
...
// Hiển thị giao diện soạn thảo
function editNotiUI() {
    // Tự động tính ngày và tuần hiện tại để gợi ý
    const d = new Date();
    ...
    const defaultTimeStr = `Ngày ${dateStr} - Tuần ${weekNum}`;

    const currentContent = Data.noti.content === "Hiện tại chưa có thông báo mới từ GVCN." ? "" : Data.noti.content;

    contentArea.innerHTML = `
        <div class="flex items-center mb-6"><button onclick="moThongBao()" class="bg-white p-2 rounded-xl shadow mr-3"><i class="fas fa-times text-slate-500"></i></button><h2 class="font-black text-xl text-orange-600 uppercase">SOẠN THÔNG BÁO</h2></div>
        
        <div class="bg-white p-6 rounded-[2rem] shadow-lg border space-y-5 fade-in">
            <div>
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-2 mb-2 p-2 bg-slate-100 rounded-xl border border-slate-200">
                    <button onclick="document.execCommand('bold', false, null)" class="w-10 h-10 bg-white rounded-lg shadow-sm hover:bg-slate-200 font-black" title="In đậm">B</button>
                    <button onclick="document.execCommand('italic', false, null)" class="w-10 h-10 bg-white rounded-lg shadow-sm hover:bg-slate-200 italic font-serif" title="In nghiêng">I</button>
                    
                    <div class="relative flex items-center bg-white rounded-lg shadow-sm px-2 hover:bg-slate-200" title="Đổi màu chữ">
                    ...
                    <div class="w-px bg-slate-300 mx-1 my-1"></div> <button onclick="chenAnhVaoThongBao()" class="px-3 h-10 bg-white rounded-lg shadow-sm hover:bg-slate-200 text-indigo-600 font-bold flex items-center gap-2" title="Chèn Hình Ảnh"><i class="fas fa-image"></i> Ảnh</button>
                    <button onclick="chenFileVaoThongBao()" class="px-3 h-10 bg-white rounded-lg shadow-sm hover:bg-slate-200 text-blue-600 font-bold flex items-center gap-2" title="Chèn Link/File"><i class="fas fa-link"></i> Link</button>
                </div>
            ...
            <button onclick="luuThongBaoLenServer()" class="w-full bg-orange-600 text-white py-4 rounded-2xl font-black btn-3d shadow-lg mt-2 text-lg hover:bg-orange-700 transition">ĐĂNG LÊN HỆ THỐNG</button>
        </div>
    `;
}

// Các hàm hỗ trợ chèn Ảnh và Link
function chenAnhVaoThongBao() {
    const url = prompt("Vui lòng dán đường link (URL) của hình ảnh vào đây:");
    if(url) {
...
function chenFileVaoThongBao() {
    const url = prompt("Dán đường link (Google Drive, Youtube, Link bài tập...) vào đây:");
    if(url) {
        const tenLink = prompt("Nhập tên hiển thị (Ví dụ: Nhấn vào đây để tải bài tập):", "Xem chi tiết tại đây");
        if(tenLink) {
            document.getElementById("frmNotiContent").focus();
            const htmlTag = `<a href="${url}" target="_blank">${tenLink}</a>`;
...
function checkAns(el, selected, correct, index) {
    ...
    if (selected === correct.toLowerCase()) { el.classList.add('!bg-green-100', '!border-green-500', '!text-green-800'); score += 10; } else el.classList.add('!bg-red-100', '!border-red-500', '!text-red-800');
    setTimeout(() => renderQuestion(index + 1), 1200);
}

function startTimer(seconds) {
    clearInterval(timer); let t = seconds;
    timer = setInterval(() => { let m = Math.floor(t / 60), s = t % 60; document.getElementById('quizTimer').innerText = `${m}:${s < 10 ? '0' + s : s}`; if (t <= 0) { clearInterval(timer); alert("Hết giờ làm bài!"); finishQuiz(); } t--; }, 1000);
}

async function finishQuiz() {
    clearInterval(timer); document.getElementById("quizBox").innerHTML = `<div class="text-center py-10 fade-in"><div class="text-7xl mb-6 animate-bounce">🏆</div><h3 class="text-2xl font-black text-slate-800 mb-2">ĐIỂM CỦA BẠN</h3><p class="text-6xl font-black text-indigo-600 mb-8">${score}</p><button onclick="loadSubject(curSub)" class="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black text-lg btn-3d shadow-lg w-full">Trở về Danh sách</button></div>`;
    if(currentUser.role === 'student') { await fetch(API_URL, { method: 'POST', mode: 'no-cors', body: JSON.stringify({ action: 'nop_bai', data: { id_hs: currentUser.id, subject: curSub, group: curGrp, score_earned: score } }) }); Data.log.push({ subject: curSub, group: curGrp }); }
}

async function quanLyNganHang(sub) {
    closeMenu(); curSub = sub; contentArea.innerHTML = `<div class="text-center mt-10"><i class="fas fa-spinner fa-spin text-3xl text-indigo-600"></i><p class="mt-2 font-bold text-gray-500">Đang tải dữ liệu...</p></div>`;
    const qs = await (await fetch(API_URL+"?type="+sub+"&t="+Date.now())).json(); Data[sub] = qs; const groups = [...new Set(qs.map(q => q.group))].sort(); let filterOptions = `<option value="all">-- Tất cả các bài --</option>` + groups.map(g => `<option value="${g}">${g}</option>`).join('');
...
function filterQuestions() {
    const val = document.getElementById("qFilter").value; const list = val === 'all' ? Data[curSub] : Data[curSub].filter(q => q.group === val);
    document.getElementById("listQuestions").innerHTML = list.length === 0 ? '<p class="text-center text-slate-400">Trống</p>' : list.map(q => `<div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col fade-in"><div class="flex justify-between items-start mb-2"><span class="text-[10px] font-black bg-indigo-100 text-indigo-700 px-2 py-1 rounded uppercase">${q.group}</span><div class="flex gap-2"><button onclick="renderFormCauHoi('${q.id}')" class="w-8 h-8 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center hover:bg-blue-600 hover:text-white transition"><i class="fas fa-edit"></i></button><button onclick="xoaCauHoi('${q.id}')" class="w-8 h-8 bg-red-50 text-red-600 rounded-lg flex items-center justify-center hover:bg-red-600 hover:text-white transition"><i class="fas fa-trash"></i></button></div></div><div class="font-bold text-slate-800 text-base">${parseImg(q.question)}</div></div>`).join('');
}
...
async function luuCauHoi(id) {
    ...
    if(!data.group || !data.question) return alert("Vui lòng điền đủ Tên bài và Câu hỏi!"); document.getElementById('loader').style.display = 'flex';
    await fetch(API_URL, { method:'POST', mode:'no-cors', body:JSON.stringify({ action: id ? 'sua_cau_hoi' : 'them_cau_hoi', data: data }) });
    alert("Lưu thành công!"); document.getElementById('loader').style.display = 'none'; quanLyNganHang(curSub);
}

async function xoaCauHoi(id) {
    if(confirm("Thầy có chắc chắn muốn xóa câu hỏi này không?")) { document.getElementById('loader').style.display = 'flex'; await fetch(API_URL, { method:'POST', mode:'no-cors', body:JSON.stringify({ action: 'xoa_cau_hoi', data: { id: id, subject: curSub } }) }); alert("Đã xóa!"); document.getElementById('loader').style.display = 'none'; quanLyNganHang(curSub); }
}

async function moDonTu() {
    closeMenu(); contentArea.innerHTML = `<h2 class="text-xl font-black text-red-600 mb-4 text-center"><i class="fas fa-spinner fa-spin"></i> Đang tải...</h2>`;
    ...
    html += leaves.map(l => `<div class="bg-white p-4 rounded-xl border shadow-sm flex gap-3"><div class="w-10 h-10 bg-red-50 text-red-500 rounded-full flex justify-center items-center"><i class="fas fa-bed"></i></div><div class="flex-1"><div class="flex justify-between font-bold"><span>${l.name}</span><span class="text-xs bg-gray-100 px-2 rounded">${l.dateOff}</span></div><p class="text-sm font-bold text-indigo-600">${l.type}</p><p class="text-xs text-gray-500 italic mt-1">"${l.reason}"</p></div></div>`).join('') || '<p class="text-center text-gray-400">Trống</p>'; contentArea.innerHTML = html + "</div>";
}

async function moTienDo() {
    closeMenu(); contentArea.innerHTML = `<h2 class="text-xl font-black text-purple-600 mb-4 text-center"><i class="fas fa-spinner fa-spin"></i> Đang tính toán...</h2>`;
    const math = await (await fetch(API_URL+"?type=math")).json(); const vietnamese = await (await fetch(API_URL+"?type=vietnamese")).json();
    const total = [...new Set(math.map(x=>x.group))].length + [...new Set(vietnamese.map(x=>x.group))].length; const logs = await (await fetch(API_URL+"?type=history_all&t="+Date.now())).json();
    ...
    html += Data.hs.map(h => { const userL = logs.filter(l => l.id === h.id); const done = new Set(userL.map(l => l.subject+l.group)).size; const pct = total ? Math.round((done/total)*100) : 0; return `<div class="bg-white p-3 rounded-xl border flex justify-between items-center"><span class="font-bold text-sm">${h.name}</span><div class="w-1/2 text-right"><div class="text-xs font-bold mb-1">${done}/${total} (${pct}%)</div><div class="progress-bar"><div class="progress-fill ${pct==100?'bg-green-500':'bg-blue-500'}" style="width:${pct}%"></div></div></div></div>`; }).join(''); contentArea.innerHTML = html + "</div>";
}

function chuyenTrangQuanLy() {
    closeMenu(); let html = `<div class="flex items-center mb-6"><button onclick="veTrangChu()" class="bg-white p-2 rounded shadow mr-3"><i class="fas fa-arrow-left"></i></button><h2 class="font-black text-xl text-blue-600">QUẢN LÝ HS</h2></div><div class="space-y-3">`;
    html += Data.hs.map(h => `<div onclick="viewProfile('${h.id}')" class="bg-white p-4 rounded-xl border flex justify-between items-center cursor-pointer hover:bg-slate-50 transition"><span class="font-bold text-slate-700">${h.name}</span><span class="text-xs text-gray-500">SĐT: ${h.fatherPhone || h.motherPhone || 'Chưa có'}</span></div>`).join(''); contentArea.innerHTML = html + "</div>";
}

function viewProfile(id) {
    closeMenu(); const s = Data.hs.find(x => x.id === id); if(!s) return; document.getElementById("pfName").innerText = s.name;
    const renderPhoneActions = (phone) => {
        ...
        return `<div class="flex items-center gap-2"><span class="font-bold text-slate-800">${phone}</span><a href="tel:${cleanPhone}" class="w-8 h-8 bg-green-100 text-green-600 rounded-full flex items-center justify-center shadow-sm hover:bg-green-600 hover:text-white transition" title="Gọi điện"><i class="fas fa-phone-alt"></i></a><a href="https://zalo.me/${cleanPhone}" target="_blank" class="w-8 h-8 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center shadow-sm hover:bg-blue-600 hover:text-white transition" title="Nhắn Zalo"><i class="fas fa-comment"></i></a></div>`;
    };
    document.getElementById("pfContent").innerHTML = `<div class="space-y-4 text-sm text-slate-600"><div class="flex justify-between items-center border-b pb-2 border-slate-100"><span class="font-bold text-slate-400 uppercase text-[10px] tracking-wider">Mã Học Sinh</span><span class="font-mono font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded">${s.id}</span></div><div class="flex justify-between items-center border-b pb-2 border-slate-100"><span class="font-bold text-slate-400 uppercase text-[10px] tracking-wider">Giới tính</span><span class="font-bold text-slate-800">${s.gender || 'Chưa rõ'}</span></div><div class="flex justify-between items-center border-b pb-2 border-slate-100"><span class="font-bold text-slate-400 uppercase text-[10px] tracking-wider">Ngày sinh</span><b class="text-slate-800">${s.dob || 'Chưa cập nhật'}</b></div><div class="flex justify-between items-center border-b pb-2 border-slate-100"><span class="font-bold text-slate-400 uppercase text-[10px] tracking-wider">SĐT Cha</span>${renderPhoneActions(s.fatherPhone)}</div><div class="flex justify-between items-center border-b pb-2 border-slate-100"><span class="font-bold text-slate-400 uppercase text-[10px] tracking-wider">SĐT Mẹ</span>${renderPhoneActions(s.motherPhone)}</div><div class="flex justify-between items-center border-b pb-2 border-slate-100"><span class="font-bold text-slate-400 uppercase text-[10px] tracking-wider">Điểm Rèn Luyện</span><div class="flex items-center gap-1 bg-yellow-50 px-2 py-1 rounded text-yellow-600 font-black border border-yellow-200"><i class="fas fa-gem"></i> ${s.score || 0} Kim cương</div></div><div class="flex flex-col border-b pb-2 border-slate-100 gap-1"><span class="font-bold text-slate-400 uppercase text-[10px] tracking-wider">Địa chỉ thường trú</span><b class="text-slate-800 leading-snug">${s.address || 'Chưa cập nhật'}</b></div>${s.note ? `<div class="bg-blue-50 p-3 rounded-xl text-blue-800 italic border border-blue-100 text-xs"><i class="fas fa-info-circle mr-1"></i> Ghi chú: ${s.note}</div>` : ''}</div>`;
    document.getElementById("modalProfile").classList.remove("hidden");
}

function moXinPhep() { closeMenu(); contentArea.innerHTML = `<div class="flex items-center mb-6"><button onclick="veTrangChu()" class="bg-white p-2 rounded shadow mr-3"><i class="fas fa-arrow-left"></i></button><h2 class="font-black text-xl text-red-600">GỬI ĐƠN PHÉP</h2></div><div class="bg-white p-6 rounded-2xl shadow-sm border space-y-4"><div><label class="font-bold text-gray-600">Ngày nghỉ</label><input type="date" id="lDate" class="edit-input w-full mt-1"></div><div><label class="font-bold text-gray-600">Lý do</label><textarea id="lReason" class="edit-input w-full mt-1" rows="3" placeholder="Nhập lý do nghỉ..."></textarea></div><button onclick="sendLeave()" class="w-full bg-red-600 text-white py-3 rounded-xl font-bold btn-3d shadow-lg mt-4">GỬI ĐƠN</button></div>`; document.getElementById('lDate').valueAsDate = new Date(Date.now()+86400000); }
async function sendLeave() { const d = document.getElementById('lDate').value, r = document.getElementById('lReason').value; if(!d || !r) return alert("Vui lòng nhập đủ thông tin!"); document.getElementById('loader').style.display='flex'; try { await fetch(API_URL, { method:'POST', mode:'no-cors', body:JSON.stringify({ action:'gui_xin_phep', data:{ id:currentUser.id, name:currentUser.name, dateOff:d, type:'Nghỉ cả ngày', reason:r } }) }); alert("Gửi đơn thành công!"); veTrangChu(); } catch(e) { alert("Lỗi mạng, chưa gửi được đơn!"); } document.getElementById('loader').style.display='none'; }
