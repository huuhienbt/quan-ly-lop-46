// ==========================================
// NÃO BỘ XỬ LÝ - APP.JS (BẢN CHUẨN HOÀN CHỈNH 100%)
// Phục hồi đầy đủ chức năng bị thiếu. Cập nhật luật Đồng Hạng & Thời gian Bảng Vàng.
// ==========================================
...
// ==========================================
// 💬 TÍNH NĂNG LỜI MUỐN NÓI & THÔNG BÁO
// ==========================================
...
// --- GÓC HỌC TẬP & BẢNG VÀNG (CẬP NHẬT THUẬT TOÁN ĐỒNG HẠNG VÀ THỜI GIAN) ---
async function moGocHocTap() { 
    ...
    // TÌM THỜI GIAN ĐẠT ĐIỂM (Chỉ tính những lần cộng/trừ điểm, loại bỏ quay 0 điểm)
    ...
    // SẮP XẾP: Điểm cao xếp trên. Bằng điểm -> Ai có thời gian NHỎ HƠN (Sớm hơn) xếp trên.
    ...
    // Gom nhóm điểm duy nhất để trao Huy chương đồng hạng
...
async function luuCauHoi(id) { 
    ...
    document.getElementById('loader').style.display = 'flex'; 
    await fetch(API_URL, { method:'POST', body:JSON.stringify({ action: id ? 'sua_cau_hoi' : 'them_cau_hoi', data: data }) }); window.isQuizDataLoaded = false; 
...
async function loadSubject(sub) { 
    ...
    if (!window.isQuizDataLoaded) {
        ...
        try {
            const [mRes, tRes, lRes] = await Promise.all([
                fetch(API_URL + "?type=math&t=" + Date.now()), fetch(API_URL + "?type=vietnamese&t=" + Date.now()), fetch(API_URL + "?type=history_all&t=" + Date.now())
            ]); 
    ...
    if(grps.length === 0) { html += `<p class="text-center text-gray-400 mt-10">Hiện chưa có bài tập nào.</p>`; } 
        ...
        grps.forEach(g => { 
            ...
            if (isDone && currentUser.role === 'student') { clickAction = `alert('Con đã hoàn thành bài tập này và điểm đã được ghi nhận rồi! Hãy làm bài khác để leo Bảng Vàng nhé!')`; } 
...
async function sendLeave() { 
    ...
    const combinedType = `${type} (${session})`; document.getElementById('loader').style.display='flex'; 
    try { await fetch(API_URL, { method:'POST', body:JSON.stringify({ action:'gui_xin_phep', data:{ id:currentUser.id, name:currentUser.name, dateOff:d, type:combinedType, reason:r } }) }); alert("Gửi đơn xin phép thành công! Giáo viên đã nhận được."); veTrangChu(); } 
    catch(e) { alert("Lỗi mạng, chưa gửi được đơn!"); } document.getElementById('loader').style.display='none'; 
}
async function moDonTu() { 
    closeMenu(); contentArea.innerHTML = `<h2 class="text-xl font-black text-red-600 mb-4 text-center"><i class="fas fa-spinner fa-spin"></i> Đang tải danh sách...</h2>`; 
    const leaves = await (await fetch(API_URL + "?type=absent_list&t=" + Date.now())).json(); 
    let html = `<div class="flex items-center mb-6"><button onclick="veTrangChu()" class="bg-white p-2 rounded-xl shadow mr-3"><i class="fas fa-arrow-left text-slate-500"></i></button><h2 class="font-black text-xl text-red-600 uppercase">HỘP THƯ ĐƠN TỪ</h2></div>`; 
...
// --- KHÔI PHỤC: QUẢN LÝ HỌC SINH & HỒ SƠ ---
