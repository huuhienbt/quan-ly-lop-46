// ==========================================
// NÃO BỘ XỬ LÝ - V68 FINAL (CHI TIẾT TIẾN ĐỘ & LOG LỖI SAI)
// ==========================================
...
let wrongAnswersLog = []; // Bộ nhớ tạm lưu các câu hỏi bé làm sai
...
function setupUI() {
    ...
    if(currentUser.role === 'admin') {
        document.getElementById('menuTeacher').innerHTML = `<div onclick="chuyenTrangQuanLy()" class="p-3 hover:bg-blue-50 rounded-lg font-bold flex items-center gap-3 cursor-pointer"><i class="fas fa-users text-blue-600 w-6"></i> QL Học sinh</div><div onclick="moThongBao()" class="p-3 hover:bg-orange-50 rounded-lg font-bold flex items-center gap-3 cursor-pointer"><i class="fas fa-bullhorn text-orange-600 w-6"></i> Quản lý Bảng tin</div><div onclick="moDonTu()" class="p-3 hover:bg-red-50 rounded-lg font-bold flex items-center gap-3 cursor-pointer"><i class="fas fa-envelope-open-text text-red-600 w-6"></i> Hộp thư xin phép</div><div onclick="moTienDo()" class="p-3 hover:bg-purple-50 rounded-lg font-bold flex items-center gap-3 cursor-pointer"><i class="fas fa-chart-line text-purple-600 w-6"></i> Tiến độ Học tập</div><div onclick="quanLyNganHang('math')" class="p-3 hover:bg-indigo-50 rounded-lg font-bold flex items-center gap-3 cursor-pointer"><i class="fas fa-calculator text-indigo-600 w-6"></i> Kho Toán</div><div onclick="quanLyNganHang('vietnamese')" class="p-3 hover:bg-green-50 rounded-lg font-bold flex items-center gap-3 cursor-pointer"><i class="fas fa-book text-green-600 w-6"></i> Kho Tiếng Việt</div>`;
        document.getElementById('menuTeacher').classList.remove('hidden'); document.getElementById('menuStudent').classList.add('hidden');
    } else {
        document.getElementById('menuStudent').innerHTML = `<div onclick="viewProfile(currentUser.id)" class="p-3 hover:bg-yellow-50 rounded-lg font-bold flex items-center gap-3 cursor-pointer"><i class="fas fa-id-card text-yellow-600 w-6"></i> Hồ sơ cá nhân</div><div onclick="moXinPhep()" class="p-3 hover:bg-red-50 rounded-lg font-bold flex items-center gap-3 cursor-pointer"><i class="fas fa-pen-to-square text-red-600 w-6"></i> Gửi đơn xin phép</div>`;
...
function renderDashboardStudent() {
    ...
    contentArea.innerHTML = `
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 fade-in"><button onclick="moThongBao()" class="col-span-1 md:col-span-2 relative h-32 bg-gradient-to-r from-orange-400 to-amber-500 rounded-3xl shadow-lg flex items-center p-6 btn-3d border-2 border-white overflow-hidden group"><div class="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center text-white text-3xl mr-4"><i class="fas fa-bullhorn"></i></div><div class="text-left"><h3 class="font-black text-2xl text-white tracking-wide">BẢNG TIN LỚP</h3><p class="text-orange-100 text-sm font-bold mt-1">Xem thông báo từ GVCN</p></div>${hasNoti ? '<span class="absolute top-4 right-4 w-4 h-4 bg-red-500 rounded-full animate-ping border-2 border-white"></span>' : ''}</button><button onclick="moGocHocTap()" class="relative h-48 bg-white border-4 border-indigo-50 rounded-3xl shadow-xl flex flex-col items-center justify-center btn-3d"><div class="w-20 h-20 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600 text-4xl mb-3"><i class="fas fa-rocket"></i></div><span class="font-black text-xl text-slate-700">GÓC HỌC TẬP</span></button><button onclick="moXinPhep()" class="relative h-48 bg-white border-4 border-red-50 rounded-3xl shadow-xl flex flex-col items-center justify-center btn-3d"><div class="w-20 h-20 bg-red-100 rounded-full flex items-center justify-center text-red-500 text-4xl mb-3"><i class="fas fa-envelope-open-text"></i></div><span class="font-black text-xl text-slate-700">GỬI ĐƠN PHÉP</span></button></div>
    `;
}

// ==========================================
// 🎯 HỆ THỐNG TIẾN ĐỘ CHUYÊN SÂU (GIÁO VIÊN)
// ==========================================
async function moTienDo() { 
    ...
    let html = `
        <div class="flex items-center mb-6"><button onclick="veTrangChu()" class="bg-white p-2 rounded shadow mr-3"><i class="fas fa-arrow-left"></i></button><h2 class="font-black text-xl text-purple-600">TIẾN ĐỘ CHUNG (${total} BÀI)</h2></div>
    ...
    html += Data.hs.map(h => { 
        ...
        // Thêm sự kiện onclick để xem chi tiết
        return `
...
window.xemChiTietTienDo = function(studentId, studentName) {
    ...
    const renderSubjectProgress = (subjectCode, groupsList) => {
        ...
        return groupsList.map(grp => {
            // Tìm xem hs đã làm bài này chưa
    ...
    contentArea.innerHTML = `
        <div class="flex items-center mb-6"><button onclick="moTienDo()" class="bg-white p-2 rounded shadow mr-3"><i class="fas fa-arrow-left"></i></button><h2 class="font-black text-xl text-purple-600 uppercase">CHI TIẾT: ${studentName}</h2></div>
...
// Hàm hiển thị Popup phân tích lỗi
...
// ==========================================
// 🎯 GÓC HỌC TẬP & LÀM BÀI TRẮC NGHIỆM (ĐÃ CHÈN LOG LỖI SAI)
// ==========================================
...
function startQuiz(group, timeMins) { 
    ...
    currentQIndex = 0; 
    score = 0; 
    wrongAnswersLog = []; // Reset bộ nhớ lỗi sai
...
function checkAns(el, selected, correct, index) { 
    ...
    if (selected === correct.toLowerCase()) { 
        ...
        // Ghi nhận lỗi sai vào bộ nhớ
        let wrongText = `<div class="bg-white p-3 rounded-xl border border-red-200 mb-3 shadow-sm">
...
async function finishQuiz() { 
    ...
    if(currentUser.role === 'student') { 
        // Gửi điểm số VÀ Cả lịch sử lỗi sai lên Server
        await fetch(API_URL, { 
            method: 'POST', mode: 'no-cors', 
            body: JSON.stringify({ 
                action: 'nop_bai', 
                data: { id_hs: currentUser.id, subject: curSub, group: curGrp, score_earned: score, details: wrongAnswersLog.join('') } 
            }) 
        }); 
