...
// Nâng cấp: Tiếng Việt chia 2 màn hình, Chèn ảnh Toán học, Thông báo bài mới, Sửa lỗi Bảng vàng thời gian
// ==========================================
...
async function login() {
    ...
    } catch(e) { document.getElementById('loader').style.display = 'none'; alert("Lỗi mạng khi kiểm tra bảo mật!"); }
}
...
function veTrangChu() { 
    ...
    if(!currentUser) moThongBao(); else if(currentUser.role === 'admin') renderDashboardAdmin(); else moThongBao(); 
}
...
// BỘ TẠO THANH CÔNG CỤ (TOOLBAR) SOẠN THẢO CHUẨN QUỐC TẾ
...
// HÀM CHÈN ẢNH VÀO BẤT KỲ EDITOR NÀO (Toán, Tiếng Việt, Bảng Tin)
...
// --- GÓC HỌC TẬP & BẢNG VÀNG CHUẨN THỜI GIAN VÀ BADGE BÀI MỚI ---
async function moGocHocTap() { 
    closeMenu(); 
    
    // TẢI DỮ LIỆU ĐỂ TÍNH TOÁN BÀI TẬP MỚI VÀ BẢNG VÀNG
    ...
    // TÌM THỜI GIAN ĐẠT ĐIỂM ĐỂ XẾP HẠNG (Ai đạt trước xếp trên)
    let studentsWithTime = Data.hs.map(s => {
        ...
        if (userLogs.length > 0) {
            ...
            let lastLog = userLogs[userLogs.length - 1]; // Lấy log nộp bài cuối cùng
    ...
    // SẮP XẾP: Điểm cao lên trên -> Nếu bằng điểm, ai có thời gian NHỎ HƠN (Sớm hơn) lên trên
    ...
    // Gom điểm để trao Huy chương (Đồng điểm thì chung 1 Huy chương)
...
function filterQuestions() { 
    ...
    sortedGroups.forEach(grp => { 
        ...
        questions.forEach((q, index) => { 
            ...
            // Lược bỏ bớt chữ [BAIDOC] khi hiển thị ở danh sách
...
// KHUNG TẠO CÂU HỎI MỚI - TÍCH HỢP SOẠN THẢO VĂN BẢN VÀ CHIA KHUNG TIẾNG VIỆT
function renderFormCauHoi(id) { 
    ...
    // XỬ LÝ NỘI DUNG NẾU LÀ TIẾNG VIỆT
    ...
    if (isTV && cauHoiHtml) {
        ...
        if (match) {
            ...
            cauHoiHtml = ""; // Trường hợp thầy chỉ nhập bài đọc mà chưa có câu hỏi
        }
    }

    let formLayout = "";
    if (isTV) {
        // Form 2 phần cho Tiếng Việt
        formLayout = `
...
async function luuCauHoi(id) { 
    ...
    // Gộp bài đọc vào câu hỏi để lưu chung vào 1 ô Google Sheets
    if (isTV) {
...
async function xoaCauHoi(id) { 
    if(confirm("Thầy có chắc chắn muốn xóa câu hỏi này không?")) { 
        document.getElementById('loader').style.display = 'flex'; await fetch(API_URL, { method:'POST', body:JSON.stringify({ action: 'xoa_cau_hoi', data: { id: id, subject: curSub } }) }); 
        window.isQuizDataLoaded = false; 
        alert("Đã xóa!"); document.getElementById('loader').style.display = 'none'; quanLyNganHang(curSub, true); 
    } 
}
...
async function loadSubject(sub) { 
    ...
    if(grps.length === 0) { html += `<p class="text-center text-gray-400 mt-10">Hiện chưa có bài tập nào.</p>`; } 
        ...
        grps.forEach(g => { 
            ...
            // Đếm số câu hỏi thực tế (bỏ qua câu chỉ chứa bài đọc mà không có đáp án)
...
function startQuiz(group, timeMins) { 
    ...
    rawQuiz.forEach(q => {
        ...
        if (match) {
            readingPassage = match[1]; // Ghi nhận bài đọc
            qText = qText.replace(match[0], '').trim(); // Lọc bỏ phần bài đọc ra khỏi câu hỏi
        } else if (qText.includes('[ĐOẠN VĂN]')) {
            readingPassage = qText.replace('[ĐOẠN VĂN]', '').trim();
            qText = "";
        }
        
        // Chỉ thêm vào danh sách thi nếu có nội dung câu hỏi hoặc có đáp án
        if (qText !== "" || q.a || q.b) {
...
// GIAO DIỆN CHIA ĐÔI MÀN HÌNH CHO MÔN TIẾNG VIỆT
...
// --- 💬 TÍNH NĂNG LỜI MUỐN NÓI & XIN PHÉP ---
...
function checkSinhNhat() {
    ...
    try { if (dobStr.includes('T')) { let dt = new Date(dobStr); bDay = dt.getDate(); bMonth = dt.getMonth() + 1; } else if (dobStr.includes('/')) { let parts = dobStr.split('/'); bDay = parseInt(parts[0]); bMonth = parseInt(parts[1]); } else if (dobStr.includes('-')) { let parts = split('-'); bDay = parseInt(parts[2]); bMonth = parseInt(parts[1]); } } catch(e) { return; } 
    let today = new Date();
