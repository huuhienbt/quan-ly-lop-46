...
function renderDashboardAdmin() {
    contentArea.innerHTML = `
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-6 fade-in">
            ...
            <button onclick="if(window.moRaDaHoatDong) window.moRaDaHoatDong()" class="bg-white p-6 rounded-2xl shadow-sm border-b-4 border-emerald-100 flex flex-col items-center gap-3 btn-3d relative overflow-hidden"><div class="absolute inset-0 bg-emerald-50 opacity-0 hover:opacity-100 transition"></div><i class="fas fa-broadcast-tower text-3xl text-emerald-500 relative z-10 animate-pulse"></i><span class="font-bold text-slate-700 relative z-10">Trạng thái</span></button>
            ...
            <button onclick="window.moDonTu()" class="bg-white p-6 rounded-2xl shadow-sm border-b-4 border-red-100 flex flex-col items-center gap-3 btn-3d"><i class="fas fa-file-signature text-3xl text-red-600"></i><span class="font-bold text-slate-700">Xin phép</span></button>
...
function getNavHtml(active) {
    if (currentUser && currentUser.role === 'admin') { 
        let title = active === 'bangtin' ? 'BẢNG TIN LỚP' : (active === 'hoctap' ? 'GÓC HỌC TẬP' : 'XIN PHÉP'); 
