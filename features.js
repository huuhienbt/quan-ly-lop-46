...
// Chứa: Góc Học Tập, Bảng Vàng, Vòng Quay, Xin Phép, Thư, Tiến Độ & Quản Lý Admin
// ==========================================

// --- 🚀 GÓC HỌC TẬP & BẢNG VÀNG CHUẨN THỜI GIAN & BADGE BÀI MỚI ---
window.moGocHocTap = async function(fromRouter = false) { 
    closeMenu(); 
...
window.filterQuestions = function() { 
    ...
    sortedGroups.forEach(grp => { 
        ...
        questions.forEach((q, index) => { 
            let qText = parseImg(q.question || ""); // FIX SẬP KHO TOÁN
...
window.renderFormCauHoi = function(id) { 
    ...
    let baiDocHtml = ""; let cauHoiHtml = q.question || "";

    if (isTV && cauHoiHtml) {
        let match = cauHoiHtml.match(/\[BAIDOC\](.*?)\[\/BAIDOC\]/s);
        if (match) {
            baiDocHtml = match[1]; cauHoiHtml = cauHoiHtml.replace(match[0], '').trim();
        } else if (cauHoiHtml.includes('[ĐOẠN VĂN]')) {
            baiDocHtml = cauHoiHtml.replace('[ĐOẠN VĂN]', '').trim(); cauHoiHtml = ""; 
        }
    }
    ...
    contentArea.innerHTML = `
        <div class="flex items-center mb-6"><button onclick="quanLyNganHang('${curSub}')" class="bg-white p-2 rounded-xl shadow mr-3 text-slate-500"><i class="fas fa-arrow-left"></i></button><h2 class="font-black text-xl text-indigo-600">${id ? 'SỬA CÂU HỎI' : 'TẠO CÂU HỎI MỚI'}</h2></div>
        <div class="bg-white p-5 rounded-3xl shadow border space-y-4 fade-in">
            ...
            <button onclick="luuCauHoi('${id || ''}')" class="w-full bg-indigo-600 text-white py-4 rounded-xl font-black btn-3d shadow-lg mt-6 text-lg hover:bg-indigo-700 transition"><i class="fas fa-save mr-2"></i> LƯU CÂU HỎI LÊN HỆ THỐNG</button>
        </div>
    `; 
};
...
window.luuCauHoi = async function(id) { 
    ...
    await fetch(API_URL, { method:'POST', body:JSON.stringify({ action: id ? 'sua_cau_hoi' : 'them_cau_hoi', data: data }) }); window.isQuizDataLoaded = false; alert("Lưu thành công!"); document.getElementById('loader').style.display = 'none'; quanLyNganHang(curSub, true); 
};
...
window.moQuanLyThu = async function() { 
    closeMenu(); 
    contentArea.innerHTML = `<div class="text-center py-10"><i class="fas fa-spinner fa-spin text-4xl text-pink-500 mb-3"></i><p class="font-bold text-slate-500">Đang tải thư...</p></div>`; 
    try {
        ...
        if(letters.length === 0) {
            ...
            letters.forEach(l => {
                ...
                let sender = isAnon ? `<span class="text-purple-600"><i class="fas fa-user-secret"></i> Ẩn danh (Thực tế: ${l.name})</span>` : `<span class="text-blue-600"><i class="fas fa-user"></i> ${l.name}</span>`;
                html += `<div class="bg-white p-5 rounded-2xl shadow-sm border-l-4 ${isAnon ? 'border-purple-500' : 'border-pink-500'}"><div class="flex justify-between items-center mb-3 pb-2 border-b border-slate-50"><div class="font-bold text-sm">${sender}</div><div class="text-[10px] text-slate-400 font-bold"><i class="fas fa-clock"></i> ${l.time}</div></div><div class="text-slate-700 bg-slate-50 p-3 rounded-xl whitespace-pre-wrap font-medium">"${l.content}"</div></div>`;
            });
            html += `</div>`;
        }
        contentArea.innerHTML = html;
    } catch(e) {
        contentArea.innerHTML = `<div class="text-center text-red-500 font-bold py-10">Lỗi tải dữ liệu hộp thư.</div>`;
    }
};
...
window.moDonTu = async function() { 
    ...
    if (leaves.length === 0) { 
        ...
        leaves.forEach(l => { 
            let fDate = l.dateOff; 
            if (fDate.includes('-')) { const parts = fDate.split('-'); if(parts.length === 3) fDate = `${parts[2]}/${parts[1]}/${parts[0]}`; } 
            if (!groupedLeaves[fDate]) groupedLeaves[fDate] = []; groupedLeaves[fDate].push(l); 
        }); 
...
window.moTienDo = async function() { 
    closeMenu(); 
    contentArea.innerHTML = `<h2 class="text-xl font-black text-purple-600 mb-4 text-center"><i class="fas fa-spinner fa-spin"></i> Đang tải dữ liệu...</h2>`; 
