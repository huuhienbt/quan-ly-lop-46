...
let checkLoginInterval = setInterval(() => { if (window.currentUser && !window.isAllDataLoaded && !window.isFetchingBackground) { window.isFetchingBackground = true; window.loadAllDataOnce(false, true).catch(e => console.log("Lỗi tải ngầm")); clearInterval(checkLoginInterval); } }, 1000);
...
setInterval(() => { if (window.currentUser && window.currentUser.role === 'admin') { if (!window.hasCheckedAdminNoti) { window.hasCheckedAdminNoti = true; window.kiemTraThongBaoAdmin(); } } else { window.hasCheckedAdminNoti = false; let bell = document.getElementById('adminNotificationBell'); if (bell) bell.remove(); } }, 2000);
        ...
        <div id="gameUI" class="fixed inset-0 z-[100] bg-slate-900 overflow-hidden flex flex-col font-sans select-none touch-none">
            ...
            <div id="mg-sky" class="flex-1 relative overflow-hidden bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-800 via-slate-900 to-black">
                ...
                <div class="absolute bottom-0 left-0 w-full h-32 flex justify-center items-end">
                    ...
                    <div id="mg-cannon" class="w-16 h-20 bg-gradient-to-t from-slate-400 to-slate-200 rounded-t-xl relative z-10 border-2 border-b-0 border-slate-500 flex justify-center pt-2 shadow-[0_0_20px_rgba(59,130,246,0.5)] transition-transform duration-75">
