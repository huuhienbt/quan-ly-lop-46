...
// ==========================================
// 1. GÓC HỌC TẬP (BẢNG VÀNG CHỈ HIỆN TOP 5 & ẨN DANH HIỆU THỪA CHO TOP 1)
// ==========================================
window.moGocHocTap = async function() { 
    ...
    let htmlTop = `
        ...
        <div class="bg-green-50/80 p-5 sm:p-6 rounded-[2rem] border-2 border-green-100 shadow-sm hover:shadow-md transition relative flex flex-col sm:flex-row justify-between min-h-[120px] group mt-5 items-center gap-4">
            <div class="absolute -top-4 -right-2 bg-green-100 border-2 border-green-200 w-12 h-12 rounded-2xl flex items-center justify-center text-green-500 text-2xl shadow-sm rotate-12 group-hover:rotate-0 transition z-10 hidden sm:flex">
    ...
    if (top5.length > 0) {
        let listHtml = top5.map((s) => {
            ...
            if (isBanned) {
                ...
                // 2. DANH HIỆU THEO MỐC ĐIỂM MỚI (CHỈ HIỂN THỊ NẾU KHÔNG PHẢI TOP 1)
                if (!isTop1) {
                    if (s.score >= 15000) { 
...
// ==========================================
// 6. QUẢN LÝ TIẾN ĐỘ THÔNG MINH (ẨN DANH HIỆU THỪA CHO TOP 1)
// ==========================================
window.calculateTitle = function(student) {
    ...
    // ƯU TIÊN 1: CHÈN DANH HIỆU ĐIỂM SỐ MỚI (15k, 10k, 5k) - CHỈ HIỂN THỊ NẾU KHÔNG PHẢI TOP 1
    let titleBadge = "";
    if (!isTop1) {
