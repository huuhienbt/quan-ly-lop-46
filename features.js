// ==========================================
// FILE: FEATURES.JS (TRÁI TIM HỆ THỐNG)
// CHỨA: Góc Học Tập, Vòng Quay, Quản Lý Bài Tập, Đơn Từ, Thư, Tiến Độ
// ==========================================
...
async function moGocHocTap() { 
    ...
    if (currentUser && currentUser.role === 'student' && !window.isQuizDataLoaded) {
        ...
        try {
            ...
            console.log("Lỗi tải kho:", e); 
        }
    }

    // Đếm số lượng bài tập chưa làm để hiển thị Badge đỏ
    let mathUnread = 0; 
    let tvUnread = 0;
    ...
    // Xác định thời gian đạt điểm cuối cùng của từng học sinh
    ...
    // Thuật toán Xếp hạng Bảng Vàng (Điểm cao -> Thời gian sớm)
...
// ==========================================
// 2. TÍNH NĂNG QUẢN LÝ KHO BÀI TẬP (ADMIN)
// ==========================================
...
function filterQuestions() { 
    ...
    sortedGroups.forEach(grp => { 
        ...
        questions.forEach((q, index) => { 
            ...
            if(qText.includes('[BAIDOC]')) {
                qText = "<span class='text-yellow-600 font-bold'>[CHỨA BÀI ĐỌC]</span> " + qText.replace(/\[BAIDOC\].*?\[\/BAIDOC\]/s, '').trim();
            }
    ...
    if (isTV) {
        formLayout = `
            <div class="mb-4 p-4 bg-yellow-50 border-2 border-yellow-200 rounded-2xl">
                <label class="text-xs font-black text-yellow-700 uppercase tracking-wider block mb-2">
                    <i class="fas fa-book-reader"></i> Khung Bài Đọc (Chỉ cần nhập ở 1 câu bất kỳ của Tuần)
                </label>
            ...
            <div class="w-full mb-4">
                <label class="text-xs font-black text-green-700 uppercase tracking-wider block mb-2">
                    <i class="fas fa-question-circle"></i> Khung Câu Hỏi
                </label>
        ...
        formLayout = `
            <div class="w-full mb-4">
                <label class="text-xs font-black text-indigo-700 uppercase tracking-wider block mb-2">
                    <i class="fas fa-calculator"></i> Nội dung câu hỏi (Hỗ trợ chèn Ảnh)
                </label>
                ${getRichTextToolbar('frmQ')}
...
// ==========================================
// 3. TÍNH NĂNG LÀM BÀI TẬP HỌC SINH (CÓ VÉ LÀM LẠI)
// ==========================================
    ...
    if(grps.length === 0) { 
        ...
        grps.forEach(g => { 
            ...
            if (isDone && currentUser.role === 'student') { 
                ...
                if(tokens > 0) { 
                    clickAction = `promptRedo('${g}', ${time})`; 
                } else { 
                    clickAction = `alert('Con đã làm bài này rồi. Hãy quay Vòng Quay Nhân Phẩm để tìm VÉ LÀM LẠI nhé!')`; 
                }
            } 
...
function promptRedo(group, time) {
    let tokens = parseInt(localStorage.getItem('redo_tokens_'+currentUser.id) || '0');
    if(tokens > 0) {
    ...
    document.getElementById('content').innerHTML = `
        ...
        <div class="flex flex-col lg:flex-row gap-6">
            <div class="lg:w-1/2 bg-[#fffbeb] p-6 sm:p-8 rounded-[2rem] border-2 border-yellow-200 shadow-inner lg:h-[75vh] overflow-y-auto relative custom-scrollbar">
                ...
                <h3 class="font-black text-yellow-800 text-xl mb-4 flex items-center gap-2 border-b-2 border-yellow-200 pb-3">
                    <i class="fas fa-book-reader text-2xl"></i> NỘI DUNG ĐỌC HIỂU
                </h3>
                <div class="text-slate-800 leading-[1.8] text-base sm:text-lg whitespace-pre-wrap font-medium pb-10 [&_img]:max-w-full [&_img]:rounded-xl [&_img]:shadow-sm [&_img]:my-4">
...
function renderQuestion(index) { 
    ...
    document.getElementById("quizBox").innerHTML = `
        <div class="${wrapperClass} fade-in">
            <div class="mb-6">
                <div class="text-sm font-black ${colorTheme} mb-3 uppercase tracking-widest bg-slate-50 inline-block px-3 py-1 rounded-lg border border-slate-200">
                    CÂU HỎI ${index + 1} / ${quiz.length}
                </div>
                <div class="text-xl sm:text-2xl font-bold text-slate-800 leading-snug [&_img]:max-w-full [&_img]:rounded-xl [&_img]:shadow-sm [&_img]:my-4">
                    ${parseImg(q.question)}
                </div>
            </div>
            <div class="space-y-4">
                ${['a','b','c','d'].filter(k => q[k]).map(key => `
                    <div onclick="checkAns(this, '${key}', '${q.correct}', ${index})" class="quiz-option p-4 sm:p-5 border-2 border-slate-100 rounded-2xl flex items-center gap-4 cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition btn-3d bg-white">
                        ...
                        <div class="font-bold text-slate-700 flex-1 text-base sm:text-lg leading-relaxed">${parseImg(q[key])}</div>
                    </div>
                `).join('')}
            </div>
        </div>
...
async function finishQuiz() { 
    ...
    document.getElementById('content').innerHTML = `
        <div class="text-center bg-white p-10 rounded-[3rem] shadow-2xl fade-in max-w-lg mx-auto mt-10 border-t-8 border-indigo-500">
            ...
            <p class="text-7xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600 mb-10 drop-shadow-sm">${score}</p>
            <button onclick="loadSubject(curSub)" class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-10 py-5 rounded-2xl font-black text-xl btn-3d shadow-lg w-full hover:scale-[1.02] transition">
                HOÀN TẤT & TRỞ VỀ
            </button>
        </div>
    `; 
    ...
    if(currentUser.role === 'student') { 
        ...
        await fetch(API_URL, { 
            method: 'POST', 
            body: JSON.stringify({ action: 'nop_bai', data: { id_hs: currentUser.id, subject: curSub, group: curGrp, score_earned: score, details: wrongAnswersLog.join('') } }) 
        }); 
...
// ==========================================
// 4. VÒNG QUAY MAY MẮN (ĐÃ FIX LỖI)
// ==========================================
async function moVongQuay() {
    ...
    let slicesHtml = PRIZES.map((p, i) => `
        <div class="absolute inset-0 flex justify-center" style="transform: rotate(${i * 60}deg);">
            <div class="pt-5 font-black text-white text-sm drop-shadow-md w-16 text-center leading-tight z-20" style="transform: rotate(0deg);">${p.text}</div>
        </div>
    `).join('');
...
function thucHienQuay() {
    ...
    let idx = 0;
    if (rand < 10) idx = 0;                 // 10%
    else if (rand < 40) idx = 1;            // 30%
    else if (rand < 60) idx = 2;            // 20%
    else if (rand < 70) idx = 3;            // 10%
    else if (rand < 80) idx = 4;            // 10%
    else idx = 5;                           // 20%
    ...
    setTimeout(async () => {
        ...
        if (prize.id === 'riddle') { 
            showRiddleModal(todayStr, uniqueGroup); 
            return; 
        }
        ...
        try {
            await fetch(API_URL, { 
                method: 'POST', 
                body: JSON.stringify({ action: 'nop_bai', data: { id_hs: currentUser.id, subject: "LuckySpin", group: uniqueGroup, score_earned: prize.netScore, details: "Quay trúng: " + prize.text } }) 
            });
...
function showRiddleModal(todayStr, uniqueGroup) {
    ...
    overlay.innerHTML = `
        <div class="bg-white p-8 rounded-[2rem] shadow-2xl max-w-sm w-full text-center border-t-8 border-purple-500 animate-[cascadeDrop_0.5s_ease-out_forwards]">
            ...
            <div class="bg-purple-50 text-purple-800 text-3xl font-black p-4 rounded-xl mb-6 shadow-inner border border-purple-200">
                ${a} + ${b} x ${c} = ?
            </div>
...
async function checkRiddle(correctAns, todayStr, uniqueGroup) {
            ...
            items.forEach(l => { 
                ...
                html += `
                    <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200 flex gap-4 hover:border-slate-300 transition">
                        <div class="w-10 h-10 ${iconBg} rounded-full flex justify-center items-center text-lg shrink-0">
                            <i class="fas ${icon}"></i>
                        </div>
                        <div class="flex-1">
...
function xemChiTietTienDo(studentId, studentName) { 
...
function xemLoiSai(studentId, subjectCode, group) { 
...
function viewProfile(id) { 
    ...
    document.getElementById('content').innerHTML = `
        <div class="flex items-center mb-6">
            <button onclick="${currentUser && currentUser.role==='admin'?'chuyenTrangQuanLy()':'veTrangChu()'}" class="bg-white p-2 rounded-xl shadow mr-3 text-slate-500"><i class="fas fa-arrow-left"></i></button>
